...
def eval_method(records, method_name, method_fn, sigma=0.15, iou_thr=0.30):
...
def main():
    ...
    methods = [
        ("Heuristic Rule-based", lambda rec: heuristic_rule_based(rec, sigma=args.sigma)),
        ("Planogram-style Row Alignment", lambda rec: planogram_row_alignment(rec, sigma=args.sigma)),
        ("Oracle Grid DSI", lambda rec: infer_missing_by_dsi(rec, sigma=args.sigma)),
    ]

    rows = []
    for name, fn in methods:
        rows.append(eval_method(records, name, fn, sigma=args.sigma, iou_thr=args.iou_thr))
    ...
    out_csv = out_dir / f"baseline_comparison_{args.split}.csv"
    df.to_csv(out_csv, index=False, encoding="utf-8-sig")

    print(df)
    print("=" * 80)
    print(f"[完成] baseline comparison saved to: {out_csv}")
    print("=" * 80)
